  ...
  return (
    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg">
      ...
      <div className="mt-6 flex rounded-lg bg-gray-100 p-1">
        <button
          ...
          className={`flex-1 rounded-md py-2 text-sm transition ${
            step === "PHONE" ? "bg-white font-medium shadow" : "text-gray-500"
          }`}
        >
          📱 Mobile OTP
        </button>
        <button
          ...
          className={`flex-1 rounded-md py-2 text-sm transition ${
            step === "EMAIL" ? "bg-white font-medium shadow" : "text-gray-500"
          }`}
        >
          ✉️ Email Login
        </button>
      ...
      <div className="mt-6">
        ...
        {step === "EMAIL" && <EmailLoginStep />}

        {step === "OTP" && (
          <LoginOtpStep
            phone={phone}
            onVerified={() => {
              // Redirect handled inside LoginOtpStep
            }}
          />
        )}
      </div>
